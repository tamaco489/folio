...
const (
	...
	//
	// A4 (210x297mm) を 150 DPI で描画すると 1240x1754 px となり、Claude の
	// 高解像度入力の上限 (長辺 2576 px) に収まるため縮小されない
	// 200 DPI にすると画素数が約 1.8 倍になり画像トークンも比例して増えるため、
	...
	//
	// スキャン PDF にもページ番号やスタンプ由来の数文字が乗ることがあるため
	// ゼロ判定ではなく閾値を置く
