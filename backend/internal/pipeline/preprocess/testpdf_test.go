...
//
// arXiv 由来の PDF は再配布不可のため testdata に置けず、依存の追加も禁じられているため最小限の PDF を組み立てる
// pipeline/pdf にも同種のヘルパがあるが _test.go に閉じており他パッケージから参照できないため、必要な分だけをここに持つ

// textPage はテキストレイヤーを持つページの内容ストリームを組み立てる
...
// blankPage は矩形だけを描くページの内容ストリームを組み立てる (スキャン画像の PDF を模す)
func blankPage() string {
