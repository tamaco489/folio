...
var ErrJobNotFound = errors.New("dynamo: job not found")

// ErrFailedNeedsReason は FAILED への遷移に errorReason が必須であることを表す
var ErrFailedNeedsReason = errors.New("dynamo: FAILED requires an error reason, use MarkFailed")
...
type JobExistsError struct {
	JobID    string
	Existing Job
	// ExistingErr は旧レコードの復元に失敗した場合の理由
	ExistingErr error
}
