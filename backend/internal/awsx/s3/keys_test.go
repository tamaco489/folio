...
func TestKeys(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"正常系_経路 A の正規化前の結果の場合_生出力と同じ階層のキーになること":                {s3.TextractDocumentKey(jobID), "work/" + jobID + "/textract/document.json"},
		"正常系_経路 B のページ単位の結果の場合_work 配下の 4 桁ゼロ埋めキーになること":        {s3.BedrockPageResultKey(jobID, 1), "work/" + jobID + "/bedrock/page-0001.json"},
		"正常系_経路 B の解釈に失敗した生応答の場合_ページ結果と同じ階層の error.json になること": {s3.BedrockPageErrorKey(jobID, 1), "work/" + jobID + "/bedrock/page-0001.error.json"},
...
func TestPageKeyLexicalOrder(t *testing.T) {
	...
	tests := map[string]func(jobID string, page int) string{
		...
		"正常系_経路 B の解釈に失敗した生応答の場合_辞書順がページ順と一致すること": s3.BedrockPageErrorKey,
	}
...
func TestKeyPrefixesAreDistinct(t *testing.T) {
	...
	derived := []string{
		...
		s3.TextractDocumentKey("job"),
		s3.BedrockPageResultKey("job", 1),
		s3.BedrockPageErrorKey("job", 1),
		s3.TextLayerKey("job"),
